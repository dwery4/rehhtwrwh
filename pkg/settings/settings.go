...
type AppSettings struct {
	...
	InputTCPConfig  tcp.TCPInputConfig
	OutputTCP       []string `json:"output-tcp"`
	OutputTCPConfig tcp.TCPOutputConfig
	OutputTCPStats  bool `json:"output-tcp-stats"`

	OutputWebSocket       []string `json:"output-ws"`
	OutputWebSocketConfig ws.WebSocketOutputConfig
	...
	OutputFileConfig   file.FileOutputConfig

	InputRAW       []string `json:"input_raw"`
	InputRAWConfig raw.RAWInputConfig
	...
	KafkaTLSConfig    kafka.KafkaTLSConfig
}
...
func init() {
	...
	flag.BoolVar(&Settings.EmitterConfig.SplitOutput, "split-output", false, "By default each output gets same traffic. If set to `true` it splits traffic equally among all outputs.")
	flag.BoolVar(&Settings.EmitterConfig.RecognizeTCPSessions, "recognize-tcp-sessions", false, "[PRO] If turned on http output will create separate worker for each TCP session. Splitting output will session based as well.")
	...
	flag.BoolVar(&Settings.EmitterConfig.PrettifyHTTP, "prettify-http", false, "If enabled, will automatically decode requests and responses with: Content-Encoding: gzip and Transfer-Encoding: chunked. Useful for debugging, in conjunction with --output-stdout")
